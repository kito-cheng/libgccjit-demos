...
int main(int argc, const char *argv[]) {
  ...
  ctxt.set_bool_option (GCC_JIT_BOOL_OPTION_DUMP_EVERYTHING, 1);
  ctxt.set_bool_option (GCC_JIT_BOOL_OPTION_KEEP_INTERMEDIATES, 1);
  ...
  while ((ch = fgetc(fp)) != EOF) {
    switch (ch) {
      case '>':
        ...
        curblock.add_assignment_op (idx, GCC_JIT_BINARY_OP_PLUS, int_one);
        break;
      case '<':
        /* Append `idx -= 1` to curblock.  */
        curblock.add_assignment_op (idx, GCC_JIT_BINARY_OP_MINUS, int_one);
        break;
      case '+':
